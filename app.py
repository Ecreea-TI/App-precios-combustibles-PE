...
def fetch_excel_data() -> List[Dict[Any, Any]]:
    ...
    try:
        ...
        # Use a mirror URL or cached version for Vercel deployment
        url = os.getenv('EXCEL_URL', "https://www.osinergmin.gob.pe/seccion/centro_documental/hidrocarburos/SCOP/SCOP-DOCS/2025/Registro-precios/Ultimos-Precios-Registrados-EVPC.xlsx")
        
        # Add custom headers for Vercel with more robust configuration
        headers = {
        ...
        # Add proxy configuration for Vercel if needed
        proxies = {}
        if os.getenv('VERCEL'):
            proxies = {
        ...
        response = requests.get(url, headers=headers, proxies=proxies, timeout=10)  # Add timeout
        response.raise_for_status()  # Raise exception for HTTP errors
    except requests.exceptions.RequestException as e:
        logger.error(f"Error setting up request: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail=f"Error al configurar la solicitud: {str(e)}"
        )
